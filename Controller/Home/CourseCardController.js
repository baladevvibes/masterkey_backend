const course_card = require("../../Models/Home/CourseCardModel");
...
const uploadFolder = path.join(__dirname, "../../uploads/course_card"); // your uploads folder

function getImagePaths() {
  const dirPath = path.join(__dirname, "../../uploads/course_card");
...
exports.DeleteCourseImagesController = async (req, res) => {
  const docs = await course_card.find();
  let arr = [];
  let factImag = [];
  docs?.forEach((el) => {
...
exports.createCourseCard = async (req, res) => {
    const ipAddress =
    ...
    try {
        ...
        await course_card.create({
            ...req.body,
            image: req.file?.path,
            ipAddress: req.ip,
        });
        res.status(200).send("Category Posted Successfully");
...
exports.getIDCourseCardMetaContentController = async (req, res) => {
  const post = await course_card.findById(req.params.id);
  res.json(post);
};
...
exports.getCourseCard = async (req, res) => {
    const ipAddress =
        req.headers["x-forwarded-for"] || req.connection.remoteAddress;
    try {
        var course = await course_card.find();
        res.status(200).json({
            status: "success",
            data: course,
            // eachPrice:
...
exports.updateCourseCard = async (req, res) => {
    const ipAddress =
        req.headers["x-forwarded-for"] || req.connection.remoteAddress;
    try {
        ...
        if (req.file) {
            updateData.image = `/uploads/course_card/${req.file.filename}`;
        }

        const updatedPost = await course_card.findByIdAndUpdate(
            req.params.id,
            updateData,
            { new: true }
        );
...
exports.deleteCourseCard = async (req, res) => {
    const id = (req.params.id)
    try {

        const coursecarddata = await course_card.findOne({ _id: id });
        if (!coursecarddata) {
            return res.status(404).send("Category not found");
        }
        await course_card.deleteOne({ _id: id });
        console.log("Deleted");
...
exports.getCourseMetaDescription = async (req, res) => {
   console.log(req.params);
   
   try {
     await course_card
       .findOne({ category: req.params.bookId })
       .then((data) => {
...
exports.createMetaCourseCardController = async (req, res) => {
